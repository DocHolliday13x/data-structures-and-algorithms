...
const validatePin = (pin) => {
  let regExPat = /^\d{4}$/; // regex pattern for 4 digit pin
  return regExPat.test(pin); // checks if pin matches pattern
};
...
const validateWord = (word) => {
  let regExPat = /^[a-zA-Z]{5,10}$/; // regex pattern for words between 5 and 10 characters (/^\w{5,10}$/; failed)
  return regExPat.test(word); // checks if word matches pattern
};
...
const hasNumber = (string) => {
  let regExPat = /[a-zA-Z]+\d+/; // regex pattern for letter followed by a digit
  return regExPat.test(string); // checks if string matches pattern
};
...
const validateEmail = (email) => {
  let regExPat = /^[a-zA-Z0-9]+(\.[a-zA-Z0-9]+)?@[a-zA-Z0-9]+\.(com|net|org)$/; // regex pattern for valid email address
  return regExPat.test(email); // checks if email matches pattern
};
...
const validatePhoneNumber = (phoneNumber) => {
  let regExPat = /^(\(\d{3}\)|\d{3})[- ]?\d{3}[- ]?\d{4}$/; // regex pattern for valid phone number
  return regExPat.test(phoneNumber); // checks if phone number matches pattern
};
