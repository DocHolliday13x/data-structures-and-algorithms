...
const sortByChildren = (charArray) => {
  charArray.sort((a, b) => {
    const numChildrenA = a.children.length;
    const numChildrenB = b.children.length;
    if (numChildrenA < numChildrenB) {
      return -1;
    } else if (numChildrenA > numChildrenB) {
      return 1;
    } else {
      const houseNameA = a.house.toUpperCase();
      const houseNameB = b.house.toUpperCase();
      if (houseNameA < houseNameB) {
        return -1;
      } else if (houseNameA > houseNameB) {
        return 1;
      } else {
        return 0;
      }
...
const findShells = (str) => {
  const pattern = /\b(sells|shells|seashells)\b/g;
